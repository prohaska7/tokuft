    ...
    )
{
    ...
    n_flush.fetch_add(1); // (void)toku_sync_fetch_and_add(&n_flush, 1);
    if (write_me) n_write_me.fetch_add(1); // (void)toku_sync_fetch_and_add(&n_write_me, 1);
    if (keep_me)  n_keep_me.fetch_add(1); // (void)toku_sync_fetch_and_add(&n_keep_me, 1);
    sleep_random();
}
